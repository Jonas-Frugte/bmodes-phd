...
\section{Point Spread Function (PSF) Effects}
\subsection{Introduction}
The PSF describes how points are smeared out due to telescope optics, atmospheric effects, etc. It is modeled as a convolution of the true image. It is typically measured by measuring the shapes of stars. They should appear perfectly spherical (they are located in the Milky Way so haven't been appreciably lensed), meaning that any ellipticity can be attributed to the PSF. The PSF changes over time (e.g. due to changes in the internal temperature of the telescope) and over position in the capture image (typically there is more distortion near the edge of an image). See figure \ref{fig:PSF_space_variability}.
\begin{figure}[h]
	\center
...
Therefore, one typically measures the ellipticity of many stars during each exposure and creates a space-variable PSF through some interpolation scheme of the deduced PSF from each star, see e.g. \cite{2004MNRAS.347.1337H} for a discussion.

\subsection{Change in ellipticity due to PSF}
...
\subsection{Effects of imperfect PSF compensation}
Clearly the PSF will never be perfectly compensated for. Ref. \cite{2008A&A...484...67P} breaks this down into 3 causes:
\begin{enumerate}
	\item \textbf{The PSF model:} the model chosen to describe the PSF will never perfectly describe it.
	\item \textbf{The interpolation scheme:} no interpolation scheme will exactly describe the PSF as a function of $\vec \theta$.
	\item \textbf{The finite information available from each star:} a star provides an image of the PSF that is noisy (photon noise TODO: what is photon noise) and pixelated.
\end{enumerate}
Still working to first order and now assuming an \textit{unweighted} quadrupole definition ($W(\theta)=1$), the error can be written as multiplicative and additive shear bias. The propagation from PSF error to shear bias can be derived in a few steps. First, work with the estimator
	\begin{equation}
